...
class RemoteCieloTest < Test::Unit::TestCase
  def setup
    @gateway = CieloGateway.new(fixtures(:cielo))

    @amount = 100
    @credit_card = credit_card('0000000000000001')
    @declined_card = credit_card('0000000000000002')
    @options = {
  ...
  def test_successful_void
    ...
    assert_success void
    assert_equal 'Operation Successful', void.message
  end
  ...
  def test_successful_verify
    ...
    assert_success response
    assert_match %r{Operation Successful}, response.message
  end

  def test_failed_verify
    response = @gateway.verify(@declined_card, @options)
    assert_failure response
    assert_match %r{Not Authorized}, response.message
  end

  # def test_invalid_login
  #   gateway = CieloGateway.new(login: '', password: '')
